...
def get_IV_mp(ticker, target_date, strike, a, T, last_stock_price):
    ...
    try:
        ...
        if yfIV < 0.0001:
            ...
            yfIV = calc_iv_backwards(last_stock_price, strike, a, T, call_price)
...
def random_price_movement(daysLeft, options_data, iv):
...
def graph_simulation(colorArray, options_data, expireTime, currentTime, iv):
    for i in range(0, len(colorArray)):
        timeArray = []
        opArray = []
        copy_options_data = copy.deepcopy(options_data)
        while currentTime <= expireTime:
            copy_options_data = random_price_movement(expireTime-currentTime, copy_options_data, iv)
...
def simulate_option_price(bound, currentTime, expireTime, options_data, iv):
    print(f"\n{bound} Price Paths:")
    averagePrice = 0
    averageOption = 0
    times0 = 0
    for i in range(0, bound):
        ...
        # opArray = []
        while currentTime <= expireTime:
            random_price_movement(expireTime-currentTime, options_data, iv)
